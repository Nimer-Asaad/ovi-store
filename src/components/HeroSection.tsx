...
export function HeroSection() {
  ...
  return (
    <section className="overflow-hidden rounded-[2rem] border border-slate-800 bg-primary text-white shadow-lift">
      ...
      <div className="relative min-h-[26rem] p-6 sm:p-8">
        <div className="absolute inset-0 bg-[linear-gradient(135deg,#0F172A,#12343B_55%,#0F766E)]" />
              ...
              <div className="mt-6 grid gap-3">
                {["تسعير حسب نوع العميل", "مخزون منخفض واضح", "طلبات جملة منظمة"].map((item) => (
                  <div key={item} className="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3">
                    ...
                    <PackageCheck className="h-5 w-5 text-secondary" aria-hidden="true" />
                  </div>
                ))}
              </div>
            </div>
